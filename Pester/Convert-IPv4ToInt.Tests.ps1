# Invoke-Pester -Path .\Pester\Convert-IPv4ToInt.Tests.ps1 -Output Detailed
Describe 'Convert-IPv4ToInt' {

    It 'Positional argmuent' {
        $r = Convert-IPv4ToInt 1.2.3.4 -ErrorAction Stop
        $r | Should -BeOfType 'System.UInt32'
        $r | Should -Be 16909060
    }
    ...
    Context TestCases1 {
        ...
        It 'Convert-IPv4ToInt -Ip <Ip> == <Int>' -TestCases $testCases1 {
            param ($Ip, $Int)
            $r = Convert-IPv4ToInt -Ip $Ip -ErrorAction Stop
            $r | Should -BeOfType 'System.UInt32'
            $r | Should -Be $Int
        }

        It 'Convert-IPv4ToInt -Ip <Ip> -Binary == <Bin>' -TestCases $testCases1 {
            param ($Ip, $Bin)
            $r = Convert-IPv4ToInt -Ip $Ip -Binary -ErrorAction Stop
    ...
    Context TestCasesSubnets1 {
        ...
        It 'Convert-IPv4ToInt -Ip <Mask> -Binary == <Bin>' -TestCases $testCasesSubnets1 {
            param ($Mask, $Bin)
            $r = Convert-IPv4ToInt -Ip $Mask -Binary -ErrorAction Stop
            $r | Should -BeOfType 'System.String'
            $r | Should -Be $Bin
        }
    }
    ...
    Context TestCasesThrow1 {
        $testCasesThrow1 = @(
            ...
            @{Ip = 33                ; Throw = '*is not a valid IPv4 address*'}
            @{Ip = '253.254.255.256' ; Throw = '*is not a valid IPv4 address*'}
            @{Ip = 'abc'             ; Throw = '*is not a valid IPv4 address*'}
        )

        It 'Convert-IPv4ToInt -Ip <Ip>  (throw)' -TestCases $testCasesThrow1 {
            param ($Ip, $Throw)
            {Convert-IPv4ToInt -Ip $Ip -ErrorAction Stop} | Should -Throw $Throw
        }
    }
}
